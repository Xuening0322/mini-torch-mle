...
def _tensor_conv2d(
    ...
    for i in prange(out_size):
        out_idx = np.empty(4, np.int32)
        to_index(i, out_shape, out_idx)
        cur_batch, cur_out_channel, cur_out_height, cur_out_width = out_idx
            ...
            for cur_kw in range(kw):
                for cur_kh in range(kh):
                    if not reverse:
                        ...
                        h_pos = cur_out_height + cur_kh
                    else:
                        w_pos = cur_out_width - cur_kw
                        h_pos = cur_out_height - cur_kh
                    if 0 <= w_pos < width and 0 <= h_pos < height:
                        in_pos = cur_batch * s1[0] + cur_in_channel * s1[1] + h_pos * s1[2] + w_pos * s1[3]
                        weight_pos = cur_out_channel * s2[0] + cur_in_channel * s2[1] + cur_kh * s2[2] + cur_kw * s2[3]
                        val += ( 
