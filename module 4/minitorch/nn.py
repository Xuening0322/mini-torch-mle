...
def tile(input: Tensor, kernel: Tuple[int, int]) -> Tuple[Tensor, int, int]:
    ...
    tile_h = height // kh
    tile_w = width // kw
    input = input.contiguous().view(batch, channel, tile_h, kh, tile_w, kw)
    input = input.permute(0, 1, 2, 4, 3, 5)
    input = input.contiguous().view(batch, channel, tile_h, tile_w, kw * kh)
...
def avgpool2d(input: Tensor, kernel: Tuple[int, int]) -> Tensor:
    ...
    input = input.mean(4).view(batch, channel, tile_h, tile_w)
    return input
    # raise NotImplementedError("Need to implement for Task 4.3")
...
def maxpool2d(input: Tensor, kernel: Tuple[int, int]) -> Tensor:
    ...
    input, tile_h, tile_w = tile(input, kernel)
    input = max(input, 4)
    return input.view(batch, channel, tile_h, tile_w)
    # raise NotImplementedError("Need to implement for Task 4.4")
