...
def _tensor_matrix_multiply(
    ...
    * Only write to global memory once per kernel.
    ...
    # Computation within each thread
    if i < out_shape[1] and j < out_shape[2]:
        temp = 0.0
        for block in range((a_shape[2] + BLOCK_DIM - 1) // BLOCK_DIM):
            # Load a segment of A and B into shared memory
            ai = block * BLOCK_DIM + pi
            bj = block * BLOCK_DIM + pj
            if ai < a_shape[2] and i < a_shape[1]:
                a_shared[pi, pj] = a_storage[
                    (batch * a_batch_stride) + (i * a_strides[1]) + ai
                ]
            else:
                a_shared[pi, pj] = 0.0
            if bj < b_shape[2] and j < b_shape[1]:
                b_shared[pi, pj] = b_storage[
                    (batch * b_batch_stride) + (bj * b_strides[1]) + j
                ]
            else:
                b_shared[pi, pj] = 0.0
            cuda.syncthreads()
        ...
        out[(batch * out_strides[0]) + (i * out_strides[1]) + j] = temp
